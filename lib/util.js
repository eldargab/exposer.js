...
exports.builder = function (name) {
  return require('./builders/' + name)
}
...
Ignore.prototype.add = function (p) {
  if (p instanceof RegExp) return this.rules.push(p)
  if (p[0] != '/') p = '**/' + p
...
Ignore.prototype.match = function (path, isDir) {
  ...
  for (var i = 0; i < this.rules.length; i++) {
    ...
    var matched = rule.match ? rule.match(path) : rule.test(path)
